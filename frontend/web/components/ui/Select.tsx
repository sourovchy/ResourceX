...
export function Select({
	...
	const filteredOptions = searchable
		? options.filter((opt) => {
				...
				const searchTarget = (labelText || String(opt.value)).toLowerCase();
				return searchTarget.includes(searchQuery.toLowerCase());
			})
		: options;

	// Reset search and focus when closing/opening
	useEffect(() => {
		if (!isOpen) {
			setSearchQuery("");
			setFocusedIndex(-1);
	...
	const handleKeyDown = (e: KeyboardEvent<HTMLDivElement>) => {
		...
		switch (e.key) {
			...
			case " ":
				if (e.key === " " && searchable) {
					break; // Allow spaces in search input
				}
				e.preventDefault();
				if (isOpen) {
		...
		>
			...
			{variant === "pill" ? (
				<div
					...
					onClick={toggleOpen}
					className={`flex items-center gap-2 rounded-full border bg-card px-4 py-2.5 text-sm font-semibold transition-all shadow-sm outline-none
						${disabled ? "cursor-not-allowed opacity-60 bg-surfaceVariant" : "cursor-pointer"}
						${isOpen ? "border-primary text-primary" : "border-border text-textSecondary hover:border-primary/40 hover:text-textPrimary"}
					`}
				>
			...
			{isOpen && (
				<div className={`absolute z-50 mt-1 overflow-hidden rounded-xl glass-surface shadow-2xl border border-borderLight/60 animate-in fade-in zoom-in-95 duration-100 flex flex-col
					${variant === "pill" ? "right-0 min-w-[160px] w-max" : "w-full"}
				`}>
					{searchable && (
								...
								return (
									<li
										...
										className={`relative flex cursor-pointer select-none items-center rounded-lg px-3 py-2 text-sm transition-all duration-150 ease-out hover:scale-[1.015] active:scale-[0.99]
											...
											${isFocused && !isSelected ? "bg-primary text-onPrimary" : ""}
											${!isSelected && !isFocused ? "text-textPrimary hover:bg-primary hover:text-onPrimary" : ""}
										`}
									>
										...
										{isSelected && (
											<span className={`absolute right-3 flex items-center ${isFocused ? "text-onPrimary" : "text-primary"}`}>
